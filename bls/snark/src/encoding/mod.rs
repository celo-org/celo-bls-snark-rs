...
pub fn encode_epoch_block_to_bits(attempt: u8, maximum_non_signers: u32, new_public_keys: &Vec<PublicKey>) -> Result<Vec<bool>, Box<dyn Error>> {
    let mut epoch_bits = vec![];
    epoch_bits.extend_from_slice(
        &(0..8).map(move |i| {
            ((attempt & u8::pow(2, i)) >> i) == 1
        }).collect::<Vec<_>>(),
    );
...
pub fn encode_epoch_block_to_bytes(attempt: u8, maximum_non_signers: u32, added_public_keys: &Vec<PublicKey>) -> Result<Vec<u8>, Box<dyn Error>> {
    Ok(bits_to_bytes(&encode_epoch_block_to_bits(attempt, maximum_non_signers, added_public_keys)?))
}
