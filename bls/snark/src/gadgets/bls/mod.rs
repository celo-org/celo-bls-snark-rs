    ...
    P: PairingGadget<E, F>,
{
    ...
    pub fn verify_partial<CS: ConstraintSystem<F>>(
        ...
        // will be empty
        let is_setup = signed_bitmap.iter().all(|bit| bit.get_value().is_none());

        // Calculate the number of non-signers
        let mut num_non_signers = 0;
        let mut num_non_signers_lc = LinearCombination::zero();
        // For each bit, increment the number of non signers
        // if the bit was a 0. We calculate both the number of non signers
        // and a linear combination over it, in order to do 2 things:
        // 1. enforce that num_non_signers < maximum_non_signers
        // 2. enforce that num_non_signers was calculated correctly from the bitmap
        for bit in signed_bitmap {
            // Update the constraints
            num_non_signers_lc += (F::one(), CS::one());
            let has_not_signed_lc = bit.lc(CS::one(), F::one().neg());
            num_non_signers_lc = num_non_signers_lc + has_not_signed_lc;

            // Update our count
            if !is_setup {
                let has_not_signed = !bit.get_value().get()?;
                num_non_signers += has_not_signed as u8;
            }
        }
        ...
        // Enforce that we have correctly counted the number of non signers
        cs.enforce(
        ...
        // Enforce that the number of non signers is within the allowed threshold
        SmallerThanGadget::<F>::enforce_smaller_than(
...
mod verify_one_message {
    ...
    fn cs_verify<E: PairingEngine, F: PrimeField, P: PairingGadget<E, F>>(
        ...
        let maximum_non_signers_plus_one =
            FpGadget::alloc(cs.ns(|| "maximum non signers plus one"), || {
                Ok(F::from(num_non_signers))
            })
            .unwrap();
    ...
    fn one_signature_ok() {
        ...
        assert_eq!(cs.num_constraints(), 18524);
